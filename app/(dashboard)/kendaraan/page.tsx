...
import { getFleetSettings, FleetSettings } from '@/lib/actions/settings';
...
import { FleetOperationalOverview } from '@/components/kendaraan/FleetOperationalOverview';
...
type ActiveTab = 'analisis' | 'log';

export default function KendaraanPage() {
  const [kendaraanList, setKendaraanList] = React.useState<Kendaraan[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [activeTab, setActiveTab] = React.useState<ActiveTab>('analisis');
  const [isSettingsOpen, setIsSettingsOpen] = React.useState(false);
  const [fleetSettings, setFleetSettings] = React.useState<FleetSettings>({
  ...
  const loadData = React.useCallback(() => {
    Promise.all([
      ...
      getFleetSettings(),
    ]).then(([kList, fSettings]) => {
      setKendaraanList(kList);
      setFleetSettings(fSettings);
      setLoading(false);
    });
  }, []);
  ...
  const TABS: { id: ActiveTab; label: string; icon: React.ReactNode }[] = [
    { id: 'analisis', label: 'Analisis Biaya & Efisiensi', icon: <BarChart3 className="w-3.5 h-3.5" /> },
    { id: 'log', label: 'Log Odometer & Trip Armada', icon: <ListOrdered className="w-3.5 h-3.5" /> },
  ];

  return (
    <div className="space-y-8">
      <PageHeader
        title="Manajemen Operasional Armada"
        ...
        actions={
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              title="Atur parameter armada: harga BBM, tarif maintenance, target efisiensi"
            >
              <Settings2 className="w-4 h-4 text-[var(--brand-primary)]" />
              <span>Pengaturan Armada</span>
            </button>
            <Link
              href="/insiden"
      ...
      {/* Fleet Status Quick Overview Cards */}
      ...
      {!loading && (
        <div className="space-y-5">
          <div className="flex gap-1 p-1 bg-[var(--bg-subtle)] border border-[var(--border)] rounded-xl w-fit">
            {TABS.map((tab) => (
              <button
                key={tab.id}
                type="button"
                ...
                className={`flex items-center gap-1.5 px-4 py-2 text-xs font-bold rounded-lg transition-all ${
                  activeTab === tab.id
                    ? 'bg-[var(--bg)] text-[var(--brand-primary)] shadow-sm border border-[var(--border)]'
      ...
      <FleetSettingsModal
        ...
        onSaved={(updated) => setFleetSettings(updated)}
      />
    </div>
  );
}
